...
SELECT
    ...
    SUM(Transported_Passengers) AS Total_Transported_Pass,
    SUM(Available_Seats) AS Total_Available_Seats,
    ROUND(AVG(Transported_Passengers / Available_Seats * 100), 2) AS Load_Factor_Percentage
FROM Maindata
GROUP BY Day_Type;
...
SELECT 
  COUNT(maindata.Departures_Performed) AS Departure_Count, 
  distancegroups.Distance_interval
FROM 
  maindata 
JOIN 
  distancegroups 
  ON maindata.Distance_group_ID = distancegroups.Distance_group_ID
GROUP BY 
  distancegroups.Distance_interval;
